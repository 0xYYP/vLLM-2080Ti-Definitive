...
- INT8 KV 是容量 / 平衡路线；当前只保留 `normal` / piecewise profile。
  ...
  固定代码下不可复现，已确认与 flashinfer 版本无关。历史重放（c805572
  代码 + 0.6.8.post1 + 260 布局）实测 4K 30.05 / 60K 20.07 / 100K 20.06 /
  125K 16.19，2026-08-07 记录（70.16 / 44.23 / 32.09）**全部永久标记为
  不可复现的历史测量**（与 flashinfer 版本无关，真实根源应为当时测量方法
  差异）；260 与 272 布局的绝对吞吐存在差异（100K 20.06 vs 16.08），布局
  影响未单独隔离。实验性 FlashInfer
...
- 图文 profile 不再限制图片数量（2026-08-23 在 cybros 双 RTX 2080 Ti 22GB、
  Qwen3.8-27B W4A16 + `max-model-len 262144` 上实测）：512² 图（约 256
  视觉 token/张）1–100 张、以及 max_pixels 1280² 图（约 980 视觉 token/张，
  100 张合计约 98K 视觉 token）20–100 张全部成功，0 OOM、0 ERROR。视觉编码
  峰值显存增量锁定在每卡约 362–388 MiB（21,356–21,384 MiB，22,528 MiB 预算
  内仍有约 1.1 GiB 余量），原因是 chunked prefill（2,048 token/块）按块复用
  编码缓冲而非驻留全部特征。`image` 显式上限现为 999；超过该上限或总上下文
  顶到 `max-model-len` 的请求会在任何显存压力出现前被 HTTP 400 拒绝。剩余
  运维注意点：`max-num-seqs 1` 下单条超大图批会独占唯一槽位直到其 prefill
  结束（例如约 256K 视觉 token 约需 2–3 分钟），所以大图批造成的是延迟而非
  稳定性问题。这些路线的 `video`/`audio` 仍保持 0 关闭。
