...
均 <1.5% 在测量波动内）。历史重放（2026-08-10，回到 c805572 代码 +
0.6.8.post1 + 260 布局，同 profile/启动参数）实测 4K 30.05 / 60K 20.07 /
100K 20.06 / 125K 16.19——2026-08-07 记录的 70.16 / 44.23 / 32.09 在当年
代码/版本/布局下仍全部不可复现，**永久标记为不可复现的历史测量**（与
flashinfer 版本无关，真实根源应为当时的测量方法差异）。注：260 布局重放
与当前 272 布局的绝对吞吐存在差异（100K 20.06 vs 16.08，约 +25%），布局
对绝对吞吐的影响尚未单独隔离。
