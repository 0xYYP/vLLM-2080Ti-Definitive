...
// vLLM 2080 Ti Definitive Edition patch: per-token-head V scale for int8 KV
template <typename T, typename = void>
struct vllm_has_v_scale_member : std::false_type {};
template <typename T>
struct vllm_has_v_scale_member<T, std::void_t<decltype(std::declval<T>().maybe_v_scale_cache)>>
    : std::true_type {};
template <typename KTraits, typename Params>
__device__ __forceinline__ void compute_sfm_v(
    const Params& params, typename KTraits::AttentionVariant variant,
    const uint32_t batch_idx, const uint32_t kv_idx_base, const uint32_t kv_head_idx,
...
#pragma unroll
    for (uint32_t mma_d = 0; mma_d < KTraits::NUM_MMA_D_VO; ++mma_d) {
      ...
      if constexpr (sizeof(typename KTraits::DTypeKV) == 1) {
        ...
        // vLLM patch: per-token-head V scale applied on the f16 B fragment.
        // B fragment (mma.m16n8k16, col-major): per-thread 4 f16 with
        //   k = 2*(tid/4)+{0,1}, n = 2*(tid%4)+{0,1}; the two m16n8k16 halves
        //   of m16n16k16 share the same k layout (n=0..7 and n=8..15).
        if constexpr (vllm_has_v_scale_member<Params>::value &&
                      variant.use_softmax) {
          const uint32_t lane_idx = threadIdx.x;
          const uint32_t k0 = kv_idx_base + mma_kv * 16 + 2 * (lane_idx / 4);
          float vs0 = params.maybe_v_scale_cache[variant.physical_scale_index(
              params, batch_idx, k0, kv_head_idx)];
          float vs1 = params.maybe_v_scale_cache[variant.physical_scale_index(
              params, batch_idx, k0 + 1, kv_head_idx)];
          half2* hb = reinterpret_cast<half2*>(b_frag);
          const half2 vs01 = __floats2half2_rn(vs0, vs1);
          hb[0] = __hmul2(hb[0], vs01);
          hb[1] = __hmul2(hb[1], vs01);
          hb[2] = __hmul2(hb[2], vs01);
          hb[3] = __hmul2(hb[3], vs01);
        }
        // end vLLM patch
      } else {
...
#pragma unroll 1
    for (uint32_t iter = 0; iter < num_iterations; ++iter) {
      ...
      // compute sfm*v
      compute_sfm_v<KTraits, Params>(params, variant, /*batch_idx=*/0,
                               kv_idx_base, kv_head_idx, &v_smem, &v_smem_offset_r, s_frag,
                               o_frag, d);
...
#pragma unroll 1
    for (uint32_t iter = 0; iter < num_iterations; ++iter) {
      ...
      // compute sfm*v
      compute_sfm_v<KTraits, Params>(params, variant, /*batch_idx=*/request_idx,
                               kv_idx_base, kv_head_idx, &v_smem, &v_smem_offset_r, s_frag,
                               o_frag, d);
