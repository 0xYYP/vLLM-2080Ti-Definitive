...
#define REGISTER_VALUE_TRANSFORM(params, value, batch_idx, qo_idx, kv_idx, qo_head_idx, kv_head_idx, ...) \
  template <typename Params, typename T> \
  __device__ __forceinline__ T ValueTransform(const Params& params, T value, uint32_t batch_idx, \
                                              uint32_t qo_idx, uint32_t kv_idx, \
                                              uint32_t qo_head_idx, uint32_t kv_head_idx) { \
    __VA_ARGS__ \
  }

#define REGISTER_PROBABILITY_TRANSFORM(params, prob, batch_idx, qo_idx, kv_idx, qo_head_idx, kv_head_idx, ...) \
  template <typename Params, typename T> \
  __device__ __forceinline__ T ProbabilityTransform(const Params& params, T prob, uint32_t batch_idx, \
                                                    uint32_t qo_idx, uint32_t kv_idx, \
                                                    uint32_t qo_head_idx, uint32_t kv_head_idx) { \
    __VA_ARGS__ \
  }
