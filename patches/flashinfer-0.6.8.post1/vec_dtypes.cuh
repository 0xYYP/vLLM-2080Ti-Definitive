...
// Generic int8_t x vec_size (used by the int8 per-token-head KV decode path).
template <size_t vec_size>
struct vec_t<int8_t, vec_size> {
  int8_t data[vec_size];

  FLASHINFER_INLINE int8_t& operator[](size_t i) { return data[i]; }
  FLASHINFER_INLINE const int8_t& operator[](size_t i) const { return data[i]; }
  FLASHINFER_INLINE int8_t* ptr() { return data; }
  FLASHINFER_INLINE void fill(int8_t val) {
#pragma unroll
    for (size_t i = 0; i < vec_size; ++i) data[i] = val;
  }
  FLASHINFER_INLINE void load(const int8_t* src) {
#pragma unroll
    for (size_t i = 0; i < vec_size; ++i) data[i] = src[i];
  }
  // page.cuh's AppendPagedKVCache* kernels call vec_t::load with a char*.
  FLASHINFER_INLINE void load(const char* src) {
    load(reinterpret_cast<const int8_t*>(src));
  }
  FLASHINFER_INLINE void store(int8_t* ptr) const {
#pragma unroll
    for (size_t i = 0; i < vec_size; ++i) ptr[i] = data[i];
  }
  FLASHINFER_INLINE void store(char* ptr) const {
    store(reinterpret_cast<int8_t*>(ptr));
  }
  template <typename T>
  FLASHINFER_INLINE void cast_from(const vec_t<T, vec_size>& src) {
    cast_from_impl(*this, src);
...
#pragma unroll
    for (size_t i = 0; i < vec_size; ++i) dst[i] = src[i];
  }
};

template <>
struct vec_cast<float, int8_t> {
  template <size_t vec_size>
  FLASHINFER_INLINE static void cast(float* dst, const int8_t* src) {
#pragma unroll
    for (size_t i = 0; i < vec_size; ++i) dst[i] = (float)src[i];
  }
};

template <>
struct vec_cast<int8_t, float> {
  template <size_t vec_size>
  FLASHINFER_INLINE static void cast(int8_t* dst, const float* src) {
#pragma unroll
    for (size_t i = 0; i < vec_size; ++i) dst[i] = (int8_t)src[i];
  }
};
...
#pragma unroll
    for (size_t i = 0; i < vec_size; ++i) dst[i] = __int2half_rn(src[i]);
  }
};

template <>
struct vec_cast<int8_t, half> {
  template <size_t vec_size>
  FLASHINFER_INLINE static void cast(int8_t* dst, const half* src) {
#pragma unroll
    for (size_t i = 0; i < vec_size; ++i) dst[i] = (int8_t)__half2float(src[i]);
  }
};
