...
inline auto DecodeSplitKVIndptr(IdType* indptr_h, uint32_t batch_size, uint32_t kv_chunk_size,
  ...
  // vllm-2080ti: with q_len_per_req > 1 (QO_LEN loop) each request contributes
  // q_len_per_req output rows; the o_indptr must expose one chunk range per
  // query row so VariableLengthMergeStates reads the tmp layout
  // [qo_idx * num_chunks + chunk] written by the kernel.
  ...
  for (uint32_t batch_idx = 0; batch_idx < batch_size; batch_idx++) {
    ...
    for (uint32_t qo = 0; qo < q_len_per_req; ++qo) {
...
inline cudaError_t DecodePlanImpl(size_t& float_workspace_size_out, size_t& int_workspace_size_out,
                                  ...
                                  uint32_t q_len_per_req, uint32_t num_qo_heads,
  ...
  // vllm-2080ti: prefer the caller-provided q_len_per_req (kernel-internal
  // multi-query QO_LEN); otherwise fall back to recovering it from the kv
  // indptr's real batch size (batch_size carries total_num_rows).
  uint32_t q_len_per_req_eff = q_len_per_req;
  uint32_t real_batch_size = batch_size;
  if (q_len_per_req_eff <= 1) {
    // vllm-2080ti: only recover the real batch when the indptr really shows a
    // shared KV prefix (indptr[1] == indptr[-1]); independent requests with
    // increasing indptr must keep real_batch_size == batch_size, otherwise the
    // o_indptr row count no longer matches paged_kv.batch_size.
    if (real_batch_size > 0 && indptr_h != nullptr &&
        indptr_h[real_batch_size] == indptr_h[1]) {
      uint32_t kv_len = indptr_h[real_batch_size];
      real_batch_size = 0;
      while (real_batch_size + 1 <= batch_size &&
             indptr_h[real_batch_size + 1] < kv_len) {
        real_batch_size++;
      }
    }
    q_len_per_req_eff = (real_batch_size > 0 && batch_size % real_batch_size == 0)
                            ? (batch_size / real_batch_size)
                            : 1U;
  }
  auto [request_indices_vec, kv_tile_indices_vec, o_indptr_vec] =
      DecodeSplitKVIndptr(indptr_h, real_batch_size, kv_chunk_size_in_pages, q_len_per_req_eff);

  AlignedAllocator int_allocator =
      MATERIALIZE ? AlignedAllocator(int_buffer, int_workspace_size_in_bytes) : AlignedAllocator();
  // vllm-2080ti: DecodeSplitKVIndptr expands per q_len_per_req row, so the
  // int workspace for request/kv_tile indices and o_indptr must cover the
  // expanded sizes (padded_batch_size is the kernel grid, not the row count).
  plan_info.request_indices_offset = int_allocator.aligned_alloc_offset(
      std::max(padded_batch_size, request_indices_vec.size()) *
          sizeof(IdType),
      16, "batch_decode_request_indices");
  plan_info.kv_tile_indices_offset = int_allocator.aligned_alloc_offset(
      std::max(padded_batch_size, kv_tile_indices_vec.size()) *
          sizeof(IdType),
      16, "batch_decode_kv_tile_indices");
  plan_info.o_indptr_offset = int_allocator.aligned_alloc_offset(
      std::max(padded_batch_size + 1, o_indptr_vec.size()) *
          sizeof(IdType),
      16, "batch_decode_o_indptr");
  ...
  if (split_kv) {
    ...
    plan_info.v_offset = float_allocator.aligned_alloc_offset(
        num_qo_heads * padded_batch_size * q_len_per_req_eff * HEAD_DIM * sizeof(float), 16,
        "batch_decode_tmp_v");
    plan_info.s_offset = float_allocator.aligned_alloc_offset(
        num_qo_heads * padded_batch_size * q_len_per_req_eff * sizeof(float), 16,
        "batch_decode_tmp_s");
...
inline cudaError_t DecodePlan(void* float_buffer, size_t float_workspace_size_in_bytes,
                              ...
                              size_t int_workspace_size_in_bytes, DecodePlanInfo& plan_info,
                              typename Params::IdType* indptr_h, uint32_t batch_size,
                              uint32_t q_len_per_req, uint32_t num_qo_heads, uint32_t page_size,
                              bool enable_cuda_graph, cudaStream_t stream,
                              WorkEstimationFunc work_estimation_func) {
  ...
  return DecodePlanImpl<true, HEAD_DIM, POS_ENCODING_MODE, AttentionVariant, Params>(
      ...
      int_workspace_size_in_bytes, plan_info, indptr_h, batch_size, q_len_per_req, num_qo_heads,
...
inline cudaError_t DecodePlanWorkspaceSize(size_t& float_workspace_size_in_bytes,
  ...
  return DecodePlanImpl<false, HEAD_DIM, POS_ENCODING_MODE, AttentionVariant, Params>(
      ...
      /*int_workspace_size_in_bytes=*/0, plan_info, indptr_h, batch_size, q_len_per_req,
      num_qo_heads, page_size, enable_cuda_graph, stream, work_estimation_func);
}
