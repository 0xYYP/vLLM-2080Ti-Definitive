...
class TopKTopPSampler(nn.Module):
    ...
    def __call__(
        ...
        # Forward variants that don't know k_max just ignore it.
...
def apply_top_k_top_p_small_k(
    ...
    """Sort-free top-k/top-p when every request's top_k <= k_max <= 64.
...
def apply_top_k_top_p(
    ...
    if k is not None and k_max is not None and k_max <= SMALL_K_MAX and logits.is_cuda:
